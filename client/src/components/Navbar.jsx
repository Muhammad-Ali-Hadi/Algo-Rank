...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { to: '/contest-history', label: 'Contest History' },
  ];
              ...
              <div className="hidden sm:flex items-center gap-1">
                {navLinks.map(link => (
                  <Link
                    key={link.to}
                    to={link.to}
                    className={`neon-nav-link ${location.pathname === link.to ? 'neon-nav-link-active' : ''}`}
                  >
                    {link.label}
                  </Link>
                ))}
              ...
              <div className="relative">
                ...
                >
                  ...
                  <span className="hidden sm:flex items-center gap-2 text-sm text-foreground font-medium">
                    ...
                    {!profile.isVerified && (
                      <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" title="Email not verified" />
                    )}
                  </span>
                  <svg
                  ...
                  >
                    ...
                    <div className="sm:hidden py-1 border-b border-border">
                      {!profile.isVerified && (
                        <Link
                          ...
                          className="block px-4 py-2.5 text-sm text-red-400 hover:bg-red-500/10 transition-colors flex items-center gap-2"
                        >
                      ...
                      {navLinks.map(link => (
                        <Link
                          ...
                          to={link.to}
                          onClick={() => setMenuOpen(false)}
                          className="block px-4 py-2.5 text-sm text-foreground hover:bg-white/[0.05] transition-colors"
                        >
                          {link.label}
                        </Link>
                      ))}
                    ...
                    <div className="py-1 border-b border-border">
                      <Link
                        to="/profile"
                        onClick={() => setMenuOpen(false)}
                        className="block px-4 py-2.5 text-sm text-foreground hover:bg-white/[0.05] transition-colors flex items-center gap-2"
                      >
                      ...
                      {!profile.isVerified && (
                        <Link
                          to="/verify-email"
                          onClick={() => setMenuOpen(false)}
                          className="block px-4 py-2.5 text-sm text-red-400 hover:bg-red-500/10 transition-colors flex items-center gap-2 border-t border-border"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z"></path></svg>
                          Verify Email
                        </Link>
